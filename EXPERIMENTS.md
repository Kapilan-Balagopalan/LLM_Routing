...
| `experiment/prompt-routing` | Prompt-only 32D skyline and online routing |
...
whether prompt features outperform the previous 78D context. Use only the
fixed, outcome-free 32D prompt PCA context for both supervised skylines and all
online players. The 14 uncertainty and 64 hidden-state features are excluded.
...
The fixed prompt projection may use all eligible prompts but no answers or
outcomes. This is the same explicit transductive feature-construction convention
used by the prompt diagnostic. Numerical results remain to be generated by the
user.
