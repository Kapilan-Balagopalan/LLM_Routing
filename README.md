...
- prompt features with fake labels generated by a balanced probabilistic
  depth-four tree. Fifteen training-quantile splits of prompt PC1 create 16
  similarly supported leaves. Their locally alternating probabilities follow
  an overall increasing trend, producing a nonlinear step function that a
  monotonic linear logit cannot represent.
...
leaf residuals with 95% intervals, `synthetic_tree_leaf_residuals.png`, and a
portable `holdout-residual-results.zip` bundle. This command does not run an LLM
or an online routing experiment.
