...
The residual-diagnostics branch also provides a focused 1:1 train-validation
...
- prompt features with fake labels generated by a 50-tree, depth-four random-
  forest teacher. It uses up to the first 12 prompt components and learns a
  deterministic target containing pairwise interactions and oscillating terms;
  real outcomes and validation observations are not used to fit the teacher.
...
The additional `residual_predictability.png` applies the same visual instrument
to all three scenarios. Within the training half, out-of-fold logistic
predictions create honest residual targets. An Extra Trees regressor and a
linear Ridge comparator learn those targets without validation data. Validation
examples are then ordered by the signed forest-predicted residual and divided
into 10 equal-size bins. Mean observed logistic residuals, 95% intervals, the
zero line, and the ideal `y=x` correction line reveal reproducible residual
structure without deliberately mixing predicted positive and negative
corrections.

Results include `summary.json`, the original probability-binned residual CSV
and plot, `residual_predictability.csv`, `residual_predictability_bins.csv`,
`residual_predictability.png`, and a portable `holdout-residual-results.zip`
bundle. This command does not run an LLM or an online routing experiment.
...
- `src/llm_routing_simulation/residual_experiment.py`: 1:1 holdout logistic
  residual comparison, nonlinear residual-predictability plot, and synthetic
  forest-teacher diagnostic.
